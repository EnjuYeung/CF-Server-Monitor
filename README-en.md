...
The Go Agent source is included in `agent/`, based on upstream stable v1.0.16. Native Agent versions start at v1.1.0 and are independent of the controller version. Docker builds all 16 upstream OS/architecture targets. Installation, version discovery and automatic updates use the controller's `/agent` endpoints; GitHub Releases are not required. Metrics, probes, traffic accounting, reporting, configuration and service management retain upstream behavior.
