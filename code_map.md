...
| 离线 / 资源 / 流量 / 到期事件 | src/services/notifications/offline.js、resource.js、traffic.js、expiry.js | scheduler.js、outbox.js；A09、A10 |
...
- `test/review-regressions.test.js`：真实 HTTP/WS 的删除重导、整批回滚、入口校验、离线判定、通知队列和旧下载入口回归。
- `test/credential-change.test.js`：密码撤销、多种令牌入口、重启、事务回滚及状态保持。
...
2026-09-19 删除 `public/` 的 8 个旧 shell/PowerShell 安装、采集及卸载脚本；仅保留原生 `/agent/install.sh` 分发。`src/services/notification.js` 已按职责拆分，`src/utils/latestReportCache.js`、`serverBilling.js` 和前端独立探测校验实现已被单一所有者或 `src/shared/` 替代。旧 Hub 内部 HTTP 路由及无调用者缓存 API 不再保留。
