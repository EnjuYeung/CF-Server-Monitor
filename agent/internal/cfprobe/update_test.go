...
func TestSelectLatestStableRelease(t *testing.T) {
	...
	releases := []agentRelease{
		testAgentRelease("v1.0.1", false, false, base, "cf-probe-linux-arm64"),
		testAgentRelease("v1.0.2", false, true, base.Add(time.Hour), assetName),
		testAgentRelease("v1.0.3", true, false, base.Add(2*time.Hour), assetName),
		testAgentRelease("v1.0.1", false, false, base.Add(3*time.Hour), assetName),
		testAgentRelease("v0.9.9", false, false, base.Add(4*time.Hour), assetName),
	}
...
func TestSelectLatestSnapshotRelease(t *testing.T) {
	...
	base := time.Date(2026, 8, 5, 12, 0, 0, 0, time.UTC)
	releases := []agentRelease{
		testAgentRelease("v9.9.9", false, false, base.Add(5*time.Hour), assetName),
		testAgentRelease("Snapshot-2608051400", true, true, base.Add(4*time.Hour), assetName),
		testAgentRelease("beta-2608051500", true, false, base.Add(6*time.Hour), assetName),
		testAgentRelease("Snapshot-2608051600", true, false, base.Add(7*time.Hour), "cf-probe-linux-arm64"),
		testAgentRelease("Snapshot-2608051200", true, false, base, assetName),
		testAgentRelease("Snapshot-2608051300", true, false, base.Add(time.Hour), assetName),
	}
...
func TestUpdateAssetDownloadURL(t *testing.T) {
	got, err := updateAssetDownloadURL("v1.2.3", "cf-probe-linux-amd64", "https://monitor.example.com/agent")
	if err != nil {
		t.Fatalf("updateAssetDownloadURL() error = %v", err)
	}
	want := "https://monitor.example.com/agent/v1.2.3/cf-probe-linux-amd64"
	if got != want {
		t.Fatalf("asset url = %q, want %q", got, want)
	}
}

func TestUpdateAssetDownloadURLUsesMirror(t *testing.T) {
	got, err := updateAssetDownloadURL("v1.2.3", "cf-probe-linux-amd64", "https://mirror.example.com/agent/")
	if err != nil {
		t.Fatalf("updateAssetDownloadURL() error = %v", err)
	}
	want := "https://mirror.example.com/agent/v1.2.3/cf-probe-linux-amd64"
	if got != want {
...
func TestRemoteConfigMD5UsesHeaderAndPreservesUpdateProxy(t *testing.T) {
	...
	a := Agent{
		cfg: Config{
			ServerID:       "sid",
			Secret:         "secret",
			ControllerURL:  "https://worker.example.com/report",
...
func testAgentRelease(tag string, prerelease, draft bool, publishedAt time.Time, assetNames ...string) agentRelease {
	assets := make([]agentReleaseAsset, 0, len(assetNames))
	for _, name := range assetNames {
		assets = append(assets, agentReleaseAsset{Name: name, Size: 1024})
	}
	return agentRelease{
		TagName:     tag,
		Draft:       draft,
