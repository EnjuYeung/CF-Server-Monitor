...
func readConfig(path string) (Config, error) {
	...
	cfg.ControllerURL = firstNonEmpty(values["CONTROLLER_URL"], values["WORKER_URL"])
	...
	cfg.DownloadURL = strings.TrimSpace(values["DOWNLOAD_URL"])
...
func writeConfig(path string, cfg Config) error {
	...
	writeKV("CONTROLLER_URL", cfg.ControllerURL)
	writeKV("WORKER_URL", cfg.ControllerURL) // Retain compatibility with existing config readers.
	...
	writeKV("DOWNLOAD_URL", cfg.DownloadURL)
	if cfg.ConfigMD5 == "" {
		cfg.ConfigMD5 = "none"
	}
