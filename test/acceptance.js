...
import { checkOfflineNodes, checkTrafficReports, checkResourceAlerts, checkExpiringServers } from '../src/services/notification.js';
...
try {
  ...
  await step('A09', '通知失败补发', '本地 Webhook 返回 503，执行离线检测和发送，再改为 200 重试', '失败记录保留，重试成功后标记送达', async () => {
    ...
    await checkOfflineNodes(controller.env.DB);await drainNotifications(controller.env.DB);
    ...
    await checkOfflineNodes(controller.env.DB);assert.equal(controller.env.DB.prepare('SELECT count(*) AS n FROM notification_outbox').first().n,1);
  ...
  await step('A15', '重启与通知持久化', '关闭并重启主控，使用原会话读取服务器、历史、未发送通知', '50 台服务器和待发送事件保留，原 JWT 仍有效', async () => {
    ...
    await controller.env.REALTIME_HUB._ensureResourceAlertSnapshotLoaded();
    assert.ok(controller.env.REALTIME_HUB.resourceAlertWindows.size > 0);
